classdef MainView < handle
    ...
    methods
        function obj = MainView()
            ...
            set(obj.h.vol_up,'Callback',{@volumn_up,obj});
            set(obj.h.vol_down,'Callback',{@volumn_down,obj});
        end
        ...
        function [basicInfo, para,outputObj] = get_info_para_output(obj)
            ...
            if isfield(basicInfo,'flim')
                ...
                if isfield(basicInfo,'strategy')
                    strategy = basicInfo.strategy;
                else
                 % nothing to do if no strategy parameter available
                end
            end  
            if isfield(basicInfo,'strategy')
                fname = sprintf('%s_%s_',basicInfo.output,strategy); 
            else
                fname = sprintf('%s_',basicInfo.output); 
            end
    ...
    methods(Access = private)
        ...
        function para = getPara(obj)
            ...
            for n = 1:numel(controlArr)
                ...
                %end
                if regexp(rawString,'[a-zA-Z]+') % 判断有字母
                    para.(lower(controlArr(n).Tag)) = rawString;
                else % 没有字母，作为纯数字处理
                    para.(lower(controlArr(n).Tag)) = str2double(rawString);
                end
        ...
        function show_initial_setting(obj)
            %obj.config = load('.\CommonFunctions\config.mat');
...
%             set(jFrame,'Maximized',1);
            ...
            set(testTypeBox,'Heights',[-1  -5]);
             ...
             h.vol_up = uicontrol(runBox,'Style','togglebutton',...
                'String','+5','Tag','vol_up');
             h.vol_down = uicontrol(runBox,'Style','togglebutton',...
                'String','-5','Tag','vol_down');
             h.run = uicontrol('Parent',runBox,...
                 'Style','pushbutton','String','RUN','Tag','Run');
             set(runBox,'Widths',[-1 -0.2 -0.2 -1.5 ]);
...
function volumn_down(~,~,obj)
try
    player = get(obj.h.calibrate,'userdata');
    stop(player);
catch
end
[x,Fs] = audioread('.\Sounds\Noise\SSN-MSP.wav');
...
x = x / 10^(5/20); % 调一次降5dB
audiowrite('.\Sounds\Noise\SSN-MSP.wav',x,Fs);
player = audioplayer(x/2,Fs);
set(obj.h.calibrate,'userdata',player);
play(player);
end
function volumn_up(~,~,obj)
try
    player = get(obj.h.calibrate,'userdata');
    stop(player);
catch
